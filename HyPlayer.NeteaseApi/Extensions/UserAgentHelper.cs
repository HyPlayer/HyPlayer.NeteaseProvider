...
public static class UserAgentHelper
{
    ...
    private static readonly Dictionary<string, string> FixedUserAgent = new();

    public static string GetRandomUserAgent(string? uaType)
    {
        if (string.IsNullOrEmpty(uaType))
        {
            uaType = "default";
        }

        if (FixedUserAgent.TryGetValue(uaType!, out var agent))
        {
            return agent;
        }
        ...
        {
            case "mobile":
                var mobileUa = userAgentList[random.Next(8)];
                FixedUserAgent[uaType] = mobileUa;
                return mobileUa;
            case "pc":
                var pcua = userAgentList[random.Next(8, 14)];
                FixedUserAgent[uaType] = pcua;
                return pcua;
            case "default":
                var defaultUa = userAgentList[random.Next(userAgentList.Length)];
                FixedUserAgent[uaType] = defaultUa;
                return defaultUa;
            default:
                return uaType!;
        }
