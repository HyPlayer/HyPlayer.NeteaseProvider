...
public abstract class OpenApiActualRequestBase : ActualRequestBase, IDictionary<string, string>
{
    private readonly Dictionary<string, string> _backingDictionary = new();

    public string? BizContent
    {
        get => _backingDictionary.TryGetValue("bizContent", out var value) ? value : null;
        set
        {
            if (value is null)
                _backingDictionary.Remove("bizContent");
            else
                _backingDictionary["bizContent"] = value;
        }
    }

    public string? AccessToken
    {
        get => _backingDictionary.TryGetValue("accessToken", out var value) ? value : null;
        set
        {
            if (value is null)
                _backingDictionary.Remove("accessToken");
            else
                _backingDictionary["accessToken"] = value;
        }
    }
